...
export class EpreuveEditorComponent {
  ...
  ngOnInit(): void {
    ...
    if (this.router.url.includes('FromCompetition') || this.router.url.includes('CreateEpreuve')) {
      this.isAdding = true;
    }

    this.jugesServices.getJugesByCompetition(this.id).subscribe({
      next: data => {
        this.AllJuges = data.data;
        console.log('AllJuges : ', data);
      }
    })
  ...
  async addEpreuve(): Promise<void> {
    this.jugesServices.getJugesByCompetition(this.idc).subscribe({
      next: data => {
        this.juge_id = data.data[0].juge_id;
        console.log(data);
        const epreuveToAdd = {
  ...
  async addEpreuve2(): Promise<void> {

    
    
    console.log('Ajout nouvelle Epreuve', this.idj);
    const epreuveToAdd = {
      epreuve_name: this.epreuve_name,
      epreuve_description: this.epreuve_description,
      epreuve_materiels: this.epreuve_materiels,
    ...
    await new Promise(resolve => setTimeout(resolve, 1000));
    this.router.navigate(['/competitions', this.id, 'epreuves']);
  }
